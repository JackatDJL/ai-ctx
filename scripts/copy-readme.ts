...
export const copyReadme = Effect.gen(function* () {
  ...
  yield* fs.makeDirectory(distPath, { recursive: true }).pipe(
    Effect.catchAll((e) =>
      Effect.fail(
        new Error(
          `Failed to create dist directory: ${Cause.pretty(Cause.fail(e))}`,
        ),
      ),
    ),
    Effect.zipRight(
      Effect.log("[README] Dist directory created successfully."),
    ),
  );

  const content: string = yield* fs.readFileString("README.md").pipe(
    Effect.map((data) => data),
    Effect.catchAll((e) =>
      Effect.fail(
        new Error(`Failed to read README.md: ${Cause.pretty(Cause.fail(e))}`),
      ),
    ),
  ...
  yield* fs
    ...
    .pipe(
      Effect.catchAll((e) =>
        Effect.fail(
          new Error(
            `Failed to write README.md to dist: ${Cause.pretty(Cause.fail(e))}`,
          ),
        ),
      ),
