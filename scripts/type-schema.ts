...
function clearSchemaTypes() {
    return Effect.gen(function* (_) {
        ...
        if (!dirExists) {
            yield* Effect.log(`[SCHEMAS] Directory ${typesDir} does not exist, no files to clear.`);
            return; // Nothing to clear
        }

        yield* Effect.log(`[SCHEMAS] Clearing files from ${typesDir} based on criteria...`);

        // Read all files in the directory
        const files = yield* fs.readDirectory(typesDir).pipe(
            Effect.catchAll((e) =>
                Effect.fail(new Error(`Failed to read directory ${typesDir}: ${Cause.pretty(Cause.fail(e))}`))
            )
        );
        ...
        yield* Effect.forEach(tsFiles, (file) =>
            Effect.gen(function* (_) {
                ...
                const content = yield* fs.readFileString(fullPath).pipe(
                    Effect.catchAll((e) => {
                        ...
                        return Effect.logError(`Failed to read file ${fullPath} for clearing check`, e).pipe(
                ...
                const hasSingleConfigurationInterface =
                    exportedInterfaces.length === 1 && exportedInterfaces[0].includes("Configuration");

                if (hasSingleConfigurationInterface) {
                    yield* Effect.log(`[SCHEMAS] File ${file} matches clearing criteria.`);
        ...
        yield* Effect.forEach(filesToDelete, (filePath) =>
            fs.remove(filePath).pipe(
                Effect.tap(() => Effect.log(`[SCHEMAS] Deleted ${path.basename(filePath)}`)),
                Effect.catchAll((e) => {
                    // Log error but don't fail the whole clear process
                    return Effect.logError(`Failed to delete file ${path.basename(filePath)}`, e);
                })
            ), { concurrency: "unbounded" } // Delete files concurrently
        );

        if (filesToDelete.length > 0) {
             yield* Effect.log(`[SCHEMAS] Finished clearing ${filesToDelete.length} files.`);
        } else {
...
function compileSchema(name: string) {
    return Effect.gen(function* (_) {
        ...
        yield* fs.makeDirectory(path.join("src", "types"), { recursive: true })
        ...
        const schemaContent = yield* fs.readFileString(schemaPath).pipe(
            Effect.catchAll((e) =>
                Effect.fail(new Error(`Failed to read schema file ${schemaFileName}: ${Cause.pretty(Cause.fail(e))}`))
            )
        );
        ...
        try {
            ...
            return yield* Effect.fail(new Error(`Failed to parse JSON in ${schemaFileName}: ${e}`));
        }
        ...
        const typeScriptCode = yield* Effect.promise(() =>
            js2ts.compile(schema, name, { // Pass 'name' as the root type name
             ...
             Effect.catchAll((e) =>
                 Effect.fail(new Error(`json-schema-to-typescript compilation failed for ${schemaFileName}: ${e}`))
             )
        );


        // 4. Write the generated TypeScript code to the output file
        yield* fs.writeFileString(outputPath, typeScriptCode).pipe(
            Effect.catchAll((e) =>
                Effect.fail(new Error(`Failed to write output file ${outputPath}: ${Cause.pretty(Cause.fail(e))}`))
            )
        );

        yield* Effect.log(`[SCHEMAS] Successfully compiled and wrote ${outputPath}`);
...
export const compileSchemas = Effect.gen(function* () {
    ...
    const files = yield* fs.readDirectory(".").pipe(
        Effect.catchAll((e) =>
            Effect.fail(new Error(`Failed to read current directory: ${Cause.pretty(Cause.fail(e))}`))
        )
    );
    ...
    if (schemaFiles.length === 0) {
        yield* Effect.log("[SCHEMAS] No schema files found (*.schema.json) in the current directory. Nothing to compile.");
    ...
    yield* Effect.log("[SCHEMAS] All schema compilations completed successfully.");
